...
def evaluate(val_dataloader, encoder_stat, cutoff_val):
    ...
    encoder_stat.eval()  # Ensure eval mode
    ...
    with torch.no_grad():  # Disable gradient computation for faster evaluation
        for val_step, batch in enumerate(val_dataloader):
            ...
            # Convert to Python scalars immediately for accumulation (faster)
            TP += ((sim == 1) & (labels == 1)).sum().item()
            TN += ((sim == 0) & (labels == 0)).sum().item()
            FN += ((sim == 0) & (labels == 1)).sum().item()
            FP += ((sim == 1) & (labels == 0)).sum().item()

            # Batch convert tensors to CPU once (much faster than .item() per element)
            index_np = index.cpu().numpy()
            similarity_np = similarity.cpu().numpy()
            pred_lab_np = pred_lab.cpu().numpy()
            true_lab_np = true_lab.cpu().numpy()
            
            for idx, s, p, t in zip(index_np, similarity_np, pred_lab_np, true_lab_np):
                idx = int(idx)
                if idx not in mapping:
                    mapping[idx] = [float(s), float(p), float(t)]
                else:
                    if float(s) > mapping[idx][0]:
                        mapping[idx] = [float(s), float(p), float(t)]

    precision = TP / (TP + FP + 0.001)
    recall = TP / (TP + FN + 0.001)
    print("TP: {}, FP: {}, FN: {}, TN: {}. precision = {:.4f}: recall = {:.4f}".format(TP, FP, FN, TN, precision, recall))
    ...
    for i, line in mapping.items():
        if line[1] == line[2]:
            ...
            fail += 1
    print("success = {}, fail = {}, accuracy = {:.4f}".format(success, fail, success / (success + fail + 0.001)))
